        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            foreach(DataGridViewRow dr in dgvService.Rows)
            {
                ...
                if(chkbox)
                {
                    try
                    {
                        cm = new SqlCommand("IF NOT EXISTS (SELECT * FROM CashTBl WHERE serid=@serid AND TransNUMB=@TransNUMB) INSERT INTO CashTBl(TransNUMB,cid,serid,vid,price,date) VALUES(@TransNUMB,@cid,@serid,@vid,@price,@date)", dbcon.connect());
                        cm.Parameters.AddWithValue("@TransNUMB", cash.lblTransno.Text);
                        cm.Parameters.AddWithValue("@cid", cash.customerId);
                        cm.Parameters.AddWithValue("@serid", dr.Cells[1].Value.ToString());
