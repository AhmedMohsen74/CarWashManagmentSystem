...
namespace CarWashManagementSystem
{
    public partial class MainForm : Form
    {
        ...
        public void loadGrossProfit()
        {
            ...
            lblRevenus.Text = module.extractData("SELECT ISNULL(SUM(price),0) AS total FROM CashTbl WHERE date >'" + DateTime.Now.AddDays(-7) + "' AND status LIKE 'Sold' ").ToString("#,##0.00");
            ...
            double revlast7 = module.extractData("SELECT ISNULL(SUM(price),0) AS total FROM CashTbl WHERE date BETWEEN  '" + DateTime.Now.AddDays(-14) + "' AND '" + DateTime.Now.AddDays(-7) + "' AND status LIKE 'Sold' ");
