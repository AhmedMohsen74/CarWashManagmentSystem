    ...
    public partial class Report : Form
    {
        ...
        public void loadTopSelling()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT TOP 10 se.name,count(ca.serid) AS qty, ISNULL(SUM(ca.price),0) AS total FROM CashTbl AS ca JOIN ServiceTBL AS se ON ca.serid=se.id WHERE ca.date BETWEEN '" + dtFromTopSelling.Value.ToString() + "' AND '" + dtToTopSelling.Value.ToString() + "' AND status LIKE 'Sold'" +
        ...
        public void loadRevenus()
        {
            ...
            try
            {
                ...
                double total = 0;
                cm = new SqlCommand("SELECT date,ISNULL(sum(price),0) AS total FROM CashTbl WHERE date BETWEEN'" + dtFromRevenud.Value.ToString() + "' " +
        ...
        public void loadCostofGood()
        {
            try
            {
                ...
                double total = 0;
                cm = new SqlCommand("SELECT costname,cost,date FROM COSTOFGOOD WHERE date BETWEEN'" + dtFromCoG.Value.ToString() + "' AND '" + dtToCoG.Value.ToString() + "'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        {
            txtRevenus.Text = extractData("SELECT ISNULL(SUM(price),0) AS total FROM CashTbl WHERE date BETWEEN  '" + dtFromGP.Value.ToString() + "' AND '" + dtToGP.Value.ToString() + "'").ToString("#,##0.00");
            txtCoG.Text = extractData("SELECT ISNULL(SUM(cost),0) AS Cost FROM COSTOFGOOD WHERE date BETWEEN '" + dtFromGP.Value.ToString() + "' AND '" + dtToGP.Value.ToString() + "'").ToString("#,##0.00");
